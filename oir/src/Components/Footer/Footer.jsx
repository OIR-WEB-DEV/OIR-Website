    ...
    <div className="w-full grid grid-cols-1 lg:grid-cols-[1.5fr_0.6fr_0.9fr_1.6fr] bg-oirBrown gap-3 ">
      <div className="grid grid-rows-[1fr_0.6fr] gap-2 sm:gap-0 sm:grid-rows-2 bg-oirLightOrange p-6">
        <div className="flex ">
          ...
          <p className="font-regular text-sm mx-6 leading-none">
            Our mission is to facilitate international academic exchange and
            assist students in pursuing their masters degree abroad.
          </p>
        </div>
          ...
          <ul className="leading-relaxed flex lg:block">
            {contactInfo.map((contact) => (
              <a
                ...
                rel="noreferrer"
                key={contact.id}
                href={contact.hrefdata}
                className="hover:text-oirYellow"
              >
                <li className="text-xs font-medium flex content-center leading-relaxed">
                  <span className=" text-[#1C1B1F] text-sm ml-2 mr-1 sm:ml-0 sm:mr-3 flex content-center flex-wrap hover:text-oirYellow">
                    {contact.icon}
                  </span>{" "}
                  {contact.info}
                </li>
          ...
          {FooterLinks.map((link) => (
            <>
              <NavLink key={link.id} to={link.link}>
                <li className=" cursor-pointer font-regular text-xs sm:text-sm leading-6 flex hover:underline hover:decoration-dotted">
                  {link.text}
        ...
        <div className="flex lg:justify-center">
          <div className="grid w-[34%] grid-cols-4 lg:grid-cols-2 lg:grid-rows-2 text-xl gap-5 mt-2 mr-3">
            {socialLinks.map((socialLink) => (
              ...
              >
                <p className="flex content-center flex-wrap hover:text-oirYellow text-2xl">
                  {socialLink.icon}
                </p>
              </a>
            ))}
      ...
      <div className="px-6 py-5 mr-1 text-white">
        ...
        <p className="font-medium text-base flex flex-wrap leading-tight">
